...
class BaseCosts(metaclass=abc.ABCMeta):
    ...
    def get_full_cost_vector(self, trial: int, feature_indices: List[int]) -> list:
        return [self.get_one_feature_cost(ind, trial) for ind in feature_indices]
...
class BernoulliCosts(BaseCosts):
    ...
    def __init__(
        ...
        assert self.feature_vector_size == len(
            self.probabilities
...
class NonstationaryBernoulliCosts(BaseCosts):
    def __init__(
        ...
        assert (
            ...
            == len(interval_probabilities)
            == len(interval_cost_values)
        ), "There must be (change_points + 1) number of parameter lists for p and values"
        for probabilities, cost_values in zip(
            self.interval_probabilities, self.interval_cost_values
        ):
            assert self.feature_vector_size == len(
                probabilities
            ), "Probabilities must have same size as feature vector"
...
class NonstationaryGaussianCosts(BaseCosts):
    def __init__(
        ...
        assert (
            len(self.change_points) == len(interval_means) == len(interval_stds)
        ), "There must be (change_points + 1) number of parameter lists for p and values"
